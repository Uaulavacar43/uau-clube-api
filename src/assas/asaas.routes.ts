...
router.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            const offsetParam: string | undefined = req.query.offset as
                | string
                | undefined;
            const limitParam: string | undefined = req.query.limit as
                | string
                | undefined;

            const offset: number = offsetParam !== undefined ? Number(offsetParam) : 0;
            const limit: number = limitParam !== undefined ? Number(limitParam) : 100;
